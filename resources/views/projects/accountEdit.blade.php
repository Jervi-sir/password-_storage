<x-app-layout>
    @if (session('alert'))
      ...
      <x-auth-card>
          ...
          <form method="POST" action="{{ route('projectAccount.update') }}">
              @csrf
              <input type="hidden" name="id"  value="{{ $account->id }}">
              <!-- Project Name -->
              <div class="mt-4">
                  <x-label for="myInput" :value="__('Project Name')" />
                    
                  <input type="text"  placeholder="Search.." id="myInput" autocomplete="off" value="{{ $account->project()->first()->project_name }}" disabled class="bg-gray-100 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">
              </div>
